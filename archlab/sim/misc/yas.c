...
void add_ident(char *s) { add_token(TOK_IDENT, s, 0, ' '); }

void add_instr(char *s) { add_token(TOK_INSTR, s, 0, ' '); }

void add_reg(char *s) { add_token(TOK_REG, s, 0, ' '); }

void add_num(long long i) { add_token(TOK_NUM, NULL, i, ' '); }

void add_punct(char c) { add_token(TOK_PUNCT, NULL, 0, c); }
...
static void usage(char *pname)
{
    ...
    printf("   -V[n]  Generate memory initialization in Verilog format (n-way "
           "blocking)\n");
    exit(0);
}
